...
There are various advantages working with data in a long format ([explained here](http://www.jstatsoft.org/v59/i10/paper)), but what is important to know is that **most** operations on your data are easier in the long format, while **some** operations are easier in the wide format. You can switch between data formats using `pivot_longer()` (wide to long) and `pivot_wider()` (long to wide) functions.
...
Notice that the expression `RT_sec_Congruent - RT_sec_Incongruent` in `mutate()` only made sense in the context of wide format data. The lesson is that you should pay attention to the format of your data, as the format determines how to perform certain operations on your data.
