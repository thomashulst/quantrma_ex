...
### Chance exercises
There are four questions to answer for this exercise. For the first two questions, you will be sampling random numbers from a normal distribution a 1000 times. The first two questions are:
...
[Right click and download this](https://github.com/thomashulst/quantrma_lab/blob/master/data/spssdata/StatsGrades.sav?raw=true) SPSS file containing 49 students’ exam grades (let’s say it’s the final exam for a statistics class). You have likely never worked with an SPSS (.sav) file before, but still, with a quick Google search, [I was able to find a library and function to load the data into R by looking at the first answer at StackOverflow](https://stackoverflow.com/questions/33026167/reading-sav-file-into-r). Remember, you can install packages using `install.packages()` and load a library with `library()`. Make sure the .sav is in your lab directory so you can load it into R. 
