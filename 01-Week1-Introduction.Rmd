...
4. You will soon learn that the depth of this course is essentially limitless and there is almost always more than one way to answer something, or even a "correct answer" at all. This is part of the fun!
...
To include R code in your RMarkdown document, which we are going to do all the time during the labs, as well as your assignments, you should use an R code chunk. You can do that by inserting three backticks (```` ``` ````) followed by \{r} (to indicate you are going to run R code). You finish a code chunk with another three backticks (```` ``` ````). In your RMarkdown document this will look as follows:
...
## Research methods 
Discuss the questions about research methods below in your breakout room and register the answers in the R Markdown template. 
