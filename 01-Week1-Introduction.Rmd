...
This first tutorial consists of two parts. In the first part we will work on several exercises related to the readings about research methods. In the second part you will learn how to graph data using R. This tutorial assumes you have read the required readings of [Week1] (https://thomashulst.github.io/quantrma/Introduction.html) and completed the [Getting started](https://thomashulst.github.io/quantrma_ex/getting-started.html) guide. You will be completing each tutorial by writing your notes and code in an R Markdown document. You should have setup your files as described in the [Getting started section](https://thomashulst.github.io/quantrma_ex/getting-started.html). At the end of this tutorial you will upload the document with your answers to Canvas. 
...
1. copy the template file for lab 1, "Tutorial01_Graphing_StudentNumber.Rmd", and place it into the "RMarkdownsTutorials" folder (i.e. copy it out of the template folder, and into the RMarkdownsTutorials folder).
2. Rename the file to add your own student number, eg., "Tutorial01_Graphing_311476"
2. double-click the "RMarkdownsTutorials.Rproj" file
3. R Studio will now load up.
4. If you click the files tab, you will see all of the files and folders inside the "RMarkdownsTutorials" folder
5. Click on your Tutorial01 .rmd file and it will now load into the editor window. 
...
1. If you have downloaded the [RMarkdownsTutorials.zip]() file, then you already have the data file in the data folder. Assuming you are working in your main directory (your .rmd file is saved in the main folder that contains both the data and template folders), then use the following commands to load the data.
...
<span class="newthought">
On some MacOS versions, the summarytools package might require XQuartz to properly display your data summary. If you receive an error running the R code, try installing [XQuartz](xquartz.macosforge.org)
</span>
