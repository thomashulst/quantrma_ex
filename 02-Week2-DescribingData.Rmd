...
You can also use the R functions `head()` and `tail()` to print out the first 6 or last 6 rows of our data frame in the R console, or `summary()` for a quick overview of our data frame:
...
# summary() shows a brief summary of a data frame
summary(nyc_films)
```
...
You can look at the data frame in the environment tab to see what is in it, and/or you can use the `head()`/`tail()/summary()` and `nrow`/`ncol` functions again:
...
When you have completed all exercises and are happy with your progress today, please knit your document and submit it to Canvas. If you finish before the time is up, start with the required readings of Week 3, work on your assignment, or help out your fellow students. 
