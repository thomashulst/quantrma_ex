...
The sections that follow below are additional materials are very helpful when working on your assignments, but we will skip for now. For example, the section below will explain how to add a title to your graph. Or change the labels on the axes? Or add different colors, or change the font-size, or change the background? You can change all of these things by adding different lines to the existing code and this will be demonstrated below.

For this lab however, you can skip the section below and jump to the [section after](#gaplab), after completing the exercises below in your lab template.

#### `ggplot()` exercises
Complete the following exercises in your lab template:

1. The first argument of `ggplot()` should always be the data argument. In what type of form should the data be provided? (hint: `?ggplot`)
2. What function can we use to set the title of a plot? Use the ggplot [reference manual](https://ggplot2.tidyverse.org/reference/index.html), the additional materials, or Google to answer this question.

### Additional materials: `ggplot()` layers and functions
...
#### `facet_wrap()` for categories by different boroughs
...
Looking at the plot, we can now see three sets of dots, but which are countries do they represent? Let's color the dots, add a legend and make the graph prettier:
