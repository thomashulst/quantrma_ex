...
2. Make a time series plot of GDP per capita by year for the Brazil, Canada, and Mexico using the code example in Section 2.4.2.4. Make sure to update the title and labels as well.
3. Make a time series plot of Life Expectancy by year for the continents (hint: use the `group_by()` and `summarise()` functions. Look at grouping and summarizing the NYC film permits for a code example (Section 2.3.1).
