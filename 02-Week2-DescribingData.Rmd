...
Try to understand what is happening in the code above. In both cases, we first load the library `data.table`. Then, we use the function `fread()` (from the library `data.table`) to load the data and assign it to the variable `nyc_films`.^[`fread()` is not the only function in R that can read .csv files. There are other functions available, like `read.csv()`, and some of them are better suited for some situations than others. `fread()` has the advantage of being really quick and flexible.]
...
You can also use the R functions `head()` and `tail()` to print out the first 6 or last 6 rows of our data frame in the R console, or `summary()` and `str()` for a quick overview of our data frame:
...
# summary() and str() show a brief summary of a data frame
summary(nyc_films)
str(nyc_films)
```
...
You can look at the data frame in the environment tab to see what is in it, and/or you can use the `head()`/`tail()/summary()/str()` and `nrow`/`ncol` functions again:
...
### Graphing exercises
Use the code from above to solve the extra things we ask you to do for the following exercises:
...
We can easily do everything all at once using the `group_by` and `summarise` function from the `dplyr` package, as we've seen before:
